...
Future<Response> onRequest(RequestContext context) async {
  ...
  try {
    // 🚨 PERBAIKAN: currency diubah jadi currency sesuai database
    final result = await db.connection.execute(
      Sql.named(
          "SELECT balance, currency FROM fiat_wallets WHERE user_id = @uid"),
    ...
    final wallets = result.map((row) {
      return {
        'balance': double.tryParse(row[0].toString()) ?? 0.0,
        'currency': row[1],
      };
    }).toList();
